...
- **PWA**: manifest con iconos `any` y `maskable`, service worker de `@angular/service-worker`, botón de instalación y aviso de versión nueva. Los iconos se generan desde `favicon.svg`, que sigue siendo la única fuente del logotipo (`pnpm --filter @closetai/backend gen:icons`).
- **El service worker no cachea `/api` jamás**: `ngsw-config.json` no declara ningún `dataGroup` y excluye `/api/**` de las navegaciones. Cachear una foto privada o una respuesta de IA en el disco del navegador sería exactamente lo contrario de lo que promete el storage privado.
- **La actualización no se aplica sola.** Cuando el service worker descarga una versión nueva aparece un aviso y el usuario decide: recargar en mitad de un formulario a medio escribir perdería lo escrito.
- **Cámara propia con `getUserMedia`**, no sólo `capture="environment"`. Ese atributo es una sugerencia que atienden los navegadores móviles; en Chrome de escritorio abre el explorador de archivos y **no hay forma de tomar una foto**. El capturador vive en el diálogo de prenda y en la pestaña "¿Me lo compro?", con vista previa, repetir y cambio de cámara. La vista se espeja cuando apunta al usuario pero **la captura sale del fotograma crudo**: una prenda con texto saldría invertida. El `<input capture>` se conserva como respaldo para cuando el contexto no es seguro.
- **Desviación deliberada del plan: no hay nginx.** El mini PC es Windows, donde nginx significa un segundo proceso sin gestor de servicios y un segundo sitio donde desincronizar las cabeceras. En su lugar el backend sirve la SPA compilada (`SERVE_SPA`), así que el mismo origen que exige `connect-src 'self'` deja de ser configuración y pasa a ser construcción, y queda **un solo proceso** que registrar como servicio. Las fotos privadas siguen sin ser estáticas: se leen por `GET /api/media`.
- El respaldo del router de Angular vive en el filtro global de excepciones y no en un `setNotFoundHandler` propio, porque Nest registra el suyo al arrancar y Fastify sólo admite uno por prefijo. Un 404 bajo `/api`, `/health` o `/docs`, y el de un asset que falta, siguen siendo 404: devolver el `index.html` escondería el error y el service worker cachearía basura.
- **El CSP de producción sale de una sola definición** (`security-headers.ts`) y la manda helmet, que es lo único que puede declarar `frame-ancestors`. La copia del `<meta>` del `index.html` queda para el dev server, que sirve el índice sin pasar por el backend.
- **`TRUST_PROXY` es obligatorio detrás del túnel**: sin él todas las peticiones llegan desde `127.0.0.1` y el límite se aplica a todos los usuarios como si fueran uno. `HOST=127.0.0.1` deja el backend fuera de la red local aunque el cortafuegos falle.
- **Control de gasto endurecido en dos puntos que faltaban.** Cada endpoint pagado ya traía su límite, pero eran independientes y sumados dejaban pasar looks + análisis + render + etiquetado a la vez: ahora hay un **tope agregado** de 12 llamadas por minuto sobre todos ellos. Y el techo mensual era sólo por usuario, que multiplicado por N usuarios no acota nada con una única clave de OpenAI: se añadió `AI_GLOBAL_MONTHLY_BUDGET_USD` para toda la instalación, comprobado en la misma transacción `Serializable`. La reserva antes de llamar ya existía desde la Fase 0/3 y no cambió.
- `login` y `registro` bloquean cinco minutos al agotar sus cinco intentos. Sin `blockDuration` el cupo se renovaba cada minuto y la fuerza bruta seguía a 5 intentos por minuto para siempre, que con la app expuesta a internet son miles al día.
- **Despliegue en Windows con Cloudflare Tunnel**, documentado en [docs/despliegue.md](docs/despliegue.md) con sus scripts en [deploy/](deploy): tarea programada que arranca con el sistema y se reinicia sola, despliegue en un comando, copia diaria de base y fotos con retención, restauración que por defecto va **a una base de prueba** —probar no puede destruir lo que se quería proteger— y comprobación de salud que sirve de monitor.
- **Verificar:** instalas la app en el celular y en Chrome de escritorio; tomas la foto de una prenda nueva con la cámara y recorres el flujo completo (etiquetado → look → ficha) sin tocar el escritorio.
  ...
  **Verificado el 28 de agosto de 2026** en lo que no depende del mini PC: el build genera `ngsw.json` con `dataGroups` vacío y `/api/**` fuera de las navegaciones; el backend en producción sirve la SPA y sus rutas profundas, devuelve `no-cache` en `index.html`, `ngsw.json` e iconos e `immutable` en los archivos con hash, contesta 404 JSON bajo `/api` y a los assets que faltan, manda el CSP con `frame-ancestors 'none'` y HSTS, esconde Swagger y escucha sólo en loopback con `HOST=127.0.0.1`. Queda por recorrer en el mini PC lo que exige el túnel: instalación de la PWA y captura con la cámara sobre HTTPS.
